...
function EmployeeTimesheetBraidList() {
 ...
 const [formData, setFormData] = useState({
  ...
  const handleSubmit = (event) => {
    ...
    $.ajax({
      url:`${config.apiUrl}/getemployeetimesheetdatanbraidota`,
      method: "POST",
  ...
  useEffect(() => {
      ...
      if (!isLoggedIn) {
        ...
        navigate('/login');
         ...
         if (roleId == 5) {
  $.ajax({
    url: `${config.apiUrl}/getdefaultemployeetimesheetdatanbraidota/${roleId}/${userid}`,
    method: 'GET',
    headers: customHeaders,
    processData: false,
    contentType: 'application/json',
...
if (roleId == 3) {
  $.ajax({
    url: `${config.apiUrl}/getdefaultemployeetimesheetdatanbraidotaop/${roleId}/${userid}`,
      method: 'GET',
      headers: customHeaders,
    processData: false,
    contentType: 'application/json',
          ...
          <div className="col-sm-2">
            ...
            <Select
              options={lineOptions.map((option) => ({
                value: option.id,
                label: option.line_name,
              }))}
              value={
                formData.line_no
                  ? { value: formData.line_no, label: formData.line_name }
                  : null
              }
              onChange={(selectedOption) => {
                setFormData({
                  ...
                  line_name: selectedOption.label,
                });
              }}
              isSearchable
