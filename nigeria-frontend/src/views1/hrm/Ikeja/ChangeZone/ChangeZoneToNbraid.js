...
tableRef.current = $('#changezone').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      navigate('/login');
        ...
        const fetchOperatorData = () => {
            fetch(`${config.apiUrl}/ikeja/getZonedata`,{headers: customHeaders,})
 ...
 const handleZoneChange = (e) => {
  ...
  const handleMove = (entryIds) => {
    ...
    navigate(`/admin/hrm/ikeja/multiplezonetonbraid?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `${config.apiUrl}/ikeja/get_zonedata_search?where=${encodedWhere}`,
      method: 'GET',
      headers: customHeaders,
  })
  .done((response) => {
