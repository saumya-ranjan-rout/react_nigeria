...
const handleEdit = (id) => {
    navigate(`/admin/fgoutput/editikejanbraidfg/${id}`);
};
...
const handleSubmit = (event) => {
    ...
    const dateValue = adjustedDate.toISOString().split('T')[0];
      ...
      $.ajax({
        url: `${config.apiUrl}/get_fg_output_search_ikeja_nbraid`,
        method: 'POST',
        headers: customHeaders,
        data: { date: dateValue },
...
const fetchData = () => {
    $.ajax({
      url: `${config.apiUrl}/get_fg_output_default_ikeja_nbraid`,
      method: 'GET',
      headers: customHeaders,
      success: function (response) {
         // Access the timesheet results from the response object
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">FG Output <Link to="/admin/fgoutput/addnewnbraidikejafg" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new</Link>
            </CardTitle>
              </CardHeader>
              <CardBody>
...
export default IkejaNbraidFgOutput;
