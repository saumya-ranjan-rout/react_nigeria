...
function HourLossReport() {
 ...
 const token = sessionStorage.getItem('token');
  ...
  useEffect(() => {
    setLoading(true);
    document.title = 'HOURLOSS REPORT';
    ...
    if (!isLoggedIn) {
     ...
     navigate('/login');
      ...
      const filetitle =`MZ HOURLOSS REPORT DATE ${formattedToday}`;
      ...
      $.ajax({
        url:`${config.apiUrl}/gethourlossreportdirectly`,
        method: "POST",
        ...
        success: function (response) {
          ...
          setData(timesheet);
          setCurrentDate(date);
          initializeTable(timesheet,filetitle); 
  ...
  //  alert(jsonData);
    $.ajax({
     ...
     //   alert(JSON.stringify(responsee));
        const { timesheett } = responsee[0];
        const { title } = responsee[0];
        const filetitle=`MZ HOURLOSS REPORT ${title}`;
  ...
  function initializeTable(timesheet,filetitle) {
    if ($.fn.DataTable.isDataTable('#example')) {
      $('#example').DataTable().destroy();
    }
    ...
    tableRef.current = $('#example').DataTable({
      ...
      columns: [
        ...
        { 
          data: 'shift',
          render: function (data, type, row) {
            // Add a '%' symbol to the "shift" column
            return data + 'HRS';
          }
        },
         { data: 'reg' },
     ...
     // Callback after the table has been drawn
      ...
      <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
               <CardTitle tag="h5"> HOURLOSS REPORT
                <hr></hr>
                Date Range
                 <hr></hr>
               </CardTitle>
...
<DatePicker
                      ...
                      dateFormat="dd-MM-yyyy"
                      placeholderText={`${t('Select')} ${t('Td Date')}`}
                                                name="todate"
                    />
                                        </div>
                                    </>
...
<Select
          ...
          value={workerOptions.find(option => option.entryid === formData.worker_id)}
          onChange={(selectedOption) => {
            handleInputChange({ target: { name: 'worker_id', value: selectedOption.entryid } });
          }}
                  ...
                  >
                    ...
                    <h4 class="header-title"> <span class="textred" >{isSearchFetch && formData.search == 4 ? `[${formData.fromdate}-${formData.todate}]` : currentDate}</span><span class="textgreen">{isSearchFetch ? `[${formData.shift}-${formData.product_name}-${formData.line_no}-${formData.section_name}]` : ""}</span></h4>
                      ...
                      <div className="table-responsive">
                         ...
                         <thead>
                  <tr>
                  <th>{t('worker name/id')}</th>
                    <th>{t('Shift')}</th>
                    <th>{t('Date of Joining')}</th>
                    <th>{t('Total No. of Days')}</th>
                    <th>{t('Target')}</th>
                    <th>{t('Actual HR Worked')}</th>
                    <th>{t('Hourly Target')}</th>
