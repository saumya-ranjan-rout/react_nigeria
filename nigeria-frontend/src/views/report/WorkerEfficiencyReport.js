...
function WorkerEfficiencyReport() {
 ...
 const token = sessionStorage.getItem('token');
  ...
  const handleInputChange = (event) => {
    ...
    const handleSubmit = (event) => {
      ...
      $.ajax({
          url:`${config.apiUrl}/getworkerefficiencyreportData`,
          method: "POST",
          ...
          success: function (response) {
              ...
              const filetitle=`MZ WOKER EFFICIENCY REPORT ${title}`;
 ...
 setLoading(true)
        ...
        if (!isLoggedIn) {
          ...
          const filetitle =`MZ WOKER EFFICIENCY REPORT DATE ${formattedToday}`;
          setLoading(true);
            // Destroy the existing DataTable instance (if it exists)
            if ($.fn.DataTable.isDataTable('#example')) {
                $('#example').DataTable().destroy();
            ...
            const jsonData = JSON.stringify(updatedFormData);
            $.ajax({
                url:`${config.apiUrl}/gettodayworkerefficiencyreportData`,
                method: "POST",
                headers: customHeaders,
                data: jsonData,
                processData: false,
...
<Select
        ...
        />
                        ...
                        <div>
                        <h4 class="header-title"> <span class="textred" >{isSearchFetch && formData.search == 4  ? `[${formData.fromdate}-${formData.todate}]` : currentDate}</span><span class="textgreen">{isSearchFetch ? `[${formData.product_name}-${formData.line_no}-${formData.section_name}]` : ""}</span></h4>
                        </div>
