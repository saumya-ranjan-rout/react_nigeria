...
function SupervisorEfficiencyReport() {
  ...
  const customHeaders = {
    ...
    const handleSubmit = (event) => {
     event.preventDefault();
        ...
        //alert(jsonData);
        $.ajax({
            url: `${config.apiUrl}/getsupervisorefficiencyreportData`,
            method: "POST",
            ...
            success: function (response) {
                ...
                const filetitle=`MZ SUPERVISOR EFFICIENCY REPORT ${title}`;
            ...
            columns: [
                {
                    ...
                    render: function (data, type, row) {
                        if (type === 'display') {
                            const empDetails = `${row.operator_name} [ ${row.entryid} ]`;
                ...
                { 
                    ...
                    render: function (data) {
                      // Manually add "HRS" to the 'shift' property
                      return data + ' HRS';
                    },
                  },
                { data: 'tt' },
                { data: 'target' },
                {
                    ...
                    render: function (data, type, row) {
                        if (type === 'display') {
                            ...
                            const totalef = row.target * row.tt;
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if (type === 'display') {
                            ...
                            const totalComplete = complete.toFixed(2);
                            // Return the total sum for display
 ...
 setLoading(true);
        ...
        if (!isLoggedIn) {
            ...
            const filetitle =`MZ SUPERVISOR EFFICIENCY REPORT DATE ${formattedToday}`;
...
<Select
        ...
        />
                  ...
                  <select
                    ...
                    className="form-control"
                    name="worker_id"
                    value={formData.worker_id} onChange={handleInputChange}
                  >
                    <option value="">{t('Select')} {t('Worker')}</option>
                    {workerOptions.map((workerOption) => (
                      <option
                        key={workerOption.entryid}
                      ...
                      >
                        ...
                        <div className="table-responsive">
                             ...
                             <thead>
                                    <tr>
                                        <th>{t('Supervisor Name')}[{t('Entryid')}]</th>
