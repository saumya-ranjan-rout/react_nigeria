 ...
 // alert(jsonData);
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      const filetitle=`MZ EMP TIMESHEET REPORT ${title}`;
...
setLoading(true);
    ...
    if (!isLoggedIn) {
      ...
      const filetitle =`MZ EMP TIMESHEET REPORT DATE ${formattedToday}`;
      setLoading(true);
...
//alert(response.timesheet);
 ...
 const initializeTable = (timesheet, filetitle) => {
    ...
    tableRef.current = $('#example').DataTable({
      ...
      {
        extend: 'csv',
        filename: filetitle, // Set your desired filename here
      },
    ],
      ...
      columns: [
        {
          data: function(row) {
            return row.worker + ' [' + row.entry_id + ']';
          },
          render: function (data, type, row) {
            if (type === 'display') {
              // Concatenate the worker and entry_id
              const empDetails = `${row.worker} [ ${row.entry_id} ]`;
        ...
        { data: 'target' },
        {
          ...
          render: function (data, type, row) {
            if (type === 'display') {
              // Initialize a variable to store the total count of positive hours
              let count = 0;
        
              // Loop through hours HOUR1 to HOUR11 and sum them up if they are greater than 0
              for (let i = 1; i <= 11; i++) {
              ...
              const h= count;
              const p_t=row.target/row.shift;
               const l_h=row.shift-h;
               const w_h=row.shift-l_h;
               const a_t= p_t* w_h;
              // Return the total count for display
              return  a_t;
            }
            return data;
          },
        ...
        },
        {
          ...
          render: function (data, type, row) {
            if (type === 'display') {
              ...
              const efficiency = (totalSum / a_t) * 100;
        ...
        },
        {
          ...
          render: function (data, type, row) {
            if (type === 'display') {
              ...
              const ik = data.split(",");
              ik.forEach((q) => {
                if (q.trim() !== '') { // Check if the trimmed value is not empty
                  re += q.trim() + ' ';
                }
              });
              return re;
        ...
        },
        {
          ...
          render: function (data, type, row) {
            if (type === 'display') {
              ...
              let ffw = 0;
              iw.forEach((qw) => {
                const num = parseFloat(qw);
                if (!isNaN(num)) {
                  ffw += num;
                }
              });
              return ffw.toFixed(2) + ' gm'; // Rounds to two decimal places
            }
            return data;
          },
        },
      ...
      <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
               <CardTitle tag="h5"> Employee Timesheet Report
                <hr></hr>
...
<Select
        ...
        />
            ...
            <div>
            <div>
              <h4 class="header-title"> <span class="textred" >{isSearchFetch && formData.search == 4 ? `[${formData.fromdate}-${formData.todate}]` : currentDate}</span><span class="textgreen">{isSearchFetch ? `[${formData.shift}-${formData.product_name}-${formData.line_no}-${formData.section_name}]` : ""}</span></h4>
            </div>
            </div>
