...
function SectionMaster() {
  ...
  const handleEdit = (id) => {
    navigate(`/admin/master/editmonthlylaborrate/${id}`);
  };


  const handleDelete = (id) => {
    const confirmDelete = window.confirm('Are you sure you want to delete this Labor Rate?');
    if (confirmDelete) {
      fetch(`${config.apiUrl}/laborratedelete/${id}`, {
        method: 'DELETE',
        headers: customHeaders,
      })
        .then((response) => response.json())
        .then((data) => {
          console.log('Labor Rate deleted:', data);
          // Refresh the list of items
          fetchData();
        })
        .catch((error) => console.error('Error deleting Labor rate:', error));
    }
  };
...
useEffect(() => {
    document.title = 'Monthly Labor Rate';
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">Monthly Labor Rate <Link to="/admin/master/addnewmonthlylaborrate" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new
            </Link></CardTitle>
              </CardHeader>
              <CardBody>
