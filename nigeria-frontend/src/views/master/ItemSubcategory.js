...
const [isActive, setActive] = useState(false);
  ...
  const handleEdit = (id) => {
    navigate(`/admin/master/edititemsubcategory/${id}`);
  };


  const handleDelete = (id) => {
    const confirmDelete = window.confirm('Do You Really Want To Delete Item Subcategory??');
    if (confirmDelete) {
    ...
    if (tableRef.current) {
      ...
      fetch(`${config.apiUrl}/iscdelete/${id}`, {
        method: 'DELETE',
        ...
        .then((data) => {
          ...
          setServerMessage('Item Subcategory deleted successfully');
          ...
          columnDefs: [
            ...
            },
            {
              ...
              render: function (data, type, row, meta) {
                ...
                return `
                  <button class="btn btn-sm btn-warning" onclick="handleEdit(${id})"><i class="fa fa-pencil-square" aria-hidden="true"></i> Edit</button>
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">Item Subcategory <Link to="/admin/master/addnewitemsubcategory" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new
            </Link></CardTitle>
              </CardHeader>
              <CardBody>
