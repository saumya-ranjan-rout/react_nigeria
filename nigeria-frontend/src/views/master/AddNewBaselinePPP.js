...
useEffect(() => {
  document.title = 'Add Baseline PPP';
  ...
  const handleSubmit = (event) => {
    ...
    $.ajax({
      url: `${config.apiUrl}/addbaselineppp`,
      method: "POST",
      headers: customHeaders,
      data: formData,
      success: function (response) {
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">{t('Add')} {t('Baseline PPP')}</CardTitle>
                <hr></hr>
              </CardHeader>
              <CardBody>
...
<div className="form-group row">
    ...
    <div className="col-sm-6">
        <input type="number" placeholder={t('Baseline PPP')}
...
export default AddNewBaselinePPP;
