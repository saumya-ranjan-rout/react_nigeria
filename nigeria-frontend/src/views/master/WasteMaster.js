...
function WasteMaster() {
  ...
  const handleEdit = (id) => {
    navigate(`/admin/master/editwastemaster/${id}`);
  };


  const handleDelete = (id) => {
    const confirmDelete = window.confirm('Are you sure you want to delete this Waste Master?');
    if (confirmDelete) {
      fetch(`${config.apiUrl}/wastemasterdelete/${id}`, {
        method: 'DELETE',
        headers: customHeaders,
      })
        .then((response) => response.json())
        .then((data) => {
          console.log('Waste Master deleted:', data);
          // Refresh the list of items
          fetchData();
        })
        .catch((error) => console.error('Error deleting Waste Master:', error));
    }
  };
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">{t('Waste Master')} <Link to="/admin/master/addnewwastemaster" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new
            </Link></CardTitle>
              </CardHeader>
              <CardBody>
