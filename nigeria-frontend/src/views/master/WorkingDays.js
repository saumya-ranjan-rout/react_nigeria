...
function WorkingDays() {
  ...
  const handleEdit = (id) => {
    navigate(`/admin/master/editworkingdays/${id}`);
  };


  const handleDelete = (id) => {
    const confirmDelete = window.confirm('Are you sure you want to delete this Working Days?');
    if (confirmDelete) {
      fetch(`${config.apiUrl}/workingdaysdelete/${id}`, {
        method: 'DELETE',
        headers: customHeaders,
      })
        .then((response) => response.json())
        .then((data) => {
          console.log('Working Days deleted:', data);
          // Refresh the list of items
          fetchData();
        })
        .catch((error) => console.error('Error deleting Working Days:', error));
    }
  };
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">{t('Working Days')} <Link to="/admin/master/addnewworkingdays" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new
            </Link></CardTitle>
              </CardHeader>
              <CardBody>
