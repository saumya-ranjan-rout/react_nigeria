...
tableRef.current = $('#example').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      navigate('/login');
        ...
        const fetchOperatorData = () => {
            fetch( `${config.apiUrl}/getShiftdata`,{headers: customHeaders})
 ...
 //Get changezone wise machine data
  ...
  const handleMove = (entryIds) => {
    ...
    navigate(`/admin/hrm/multipleshiftchange?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `${config.apiUrl}/get_shiftdata_search?where=${encodedWhere}`,
      method: 'GET',
      headers: customHeaders,
  })
  .done((response) => {
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">Change Shift</CardTitle>
                <br/>
                <h6 >Filter</h6>
                <hr></hr>
              </CardHeader>
