...
tableRef.current = $('#changezone').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      navigate('/login');
        ...
        const fetchOperatorData = () => {
            fetch(`${config.apiUrl}/ikeja/getZonedataNbraid`,{headers: customHeaders})
  ...
  const handleMove = (entryIds) => {
    ...
    navigate(`/admin/hrm/ikeja/multiplezonetobraid?${queryParams}`);
  };
...
const encodedWhere = encodeURIComponent(where);
  $.ajax({
      url: `${config.apiUrl}/ikeja/get_zoneNbraid_search?where=${encodedWhere}`,
      method: 'GET',
      headers: customHeaders,
  })
  .done((response) => {
