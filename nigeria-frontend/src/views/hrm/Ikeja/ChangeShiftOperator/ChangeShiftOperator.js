...
function ChangeShiftOperaor() {
    ...
    if (!isLoggedIn) {
      ...
      navigate('/login');
        ...
        const fetchOperatorData = () => {
            fetch(`${config.apiUrl}/ikeja/getEmployees`,{headers: customHeaders})
  ...
  const handleMove = (entryIds) => {
    ...
    navigate(`/admin/hrm/ikeja/multipleshiftchangeoperator?${queryParams}`);
  };
