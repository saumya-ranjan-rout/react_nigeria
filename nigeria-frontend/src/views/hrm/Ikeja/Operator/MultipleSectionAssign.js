...
useEffect(() => {
  ...
  if (!isLoggedIn) {
    ...
    const fetchUsers = () => {
      $.ajax({
        url: `${config.apiUrl}/ikeja/get_users/${uid}`,
        method: 'GET',
  ...
  const handleSubmit = (event) => {
    ...
    const productPromises = array.map((value) => {
      ...
      return new Promise((resolve, reject) => {
        $.ajax({
          url: `${config.apiUrl}/ikeja/assign_check?where=${where}`,
          method: 'GET',
          headers: customHeaders,
          success: (response) => {
            if (response.length === 1) {
              $.ajax({
                url: `${config.apiUrl}/ikeja/update_assign_operator/${formData.user}?where=${where}`,
                method: 'GET',
                headers: customHeaders,
                success: () => {
                  resolve();
