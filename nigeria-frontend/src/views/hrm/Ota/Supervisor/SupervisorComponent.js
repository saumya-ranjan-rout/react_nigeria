...
export function SupervisorComponent() {
  ...
  $.ajax({
    ...
    headers: customHeaders,
    success: function (response) {
      // Assuming response is an array of timesheets
      const timesheets = response.timesheet;

      // Modify the DataTable initialization to use the received data
      ...
      tableRef.current = $('#example').DataTable({
        ...
        data: timesheets, // Use the timesheets array as data
        columns: [
          // Checkbox column
          {
            data: null,
          ...
          // Define other columns based on the properties of each timesheet object
          {
            data: null,
            className: 'row-number-column',
            render: function (data, type, row, meta) {
              return meta.row + 1; // Display row index + 1 as row number
            }
          },
          { data: 'name' },
          { data: 'entryid' },
          ...
          { data: 'shift' },
          {
            ...
            render: function (data, type, row) {
              // Render action buttons based on the properties of each timesheet object
              return `
                <button class="btn btn-sm btn-danger" onclick="DeleteOperator('${row.id}')" title="Delete"><i class="bx bxs-trash"></i></button>
              `;
            },
          },
        ],
      ...
      // Add event listener to the checkbox in the header
          ...
          <Col md="12">
            ...
            <div className="row">
             ...
             <Table responsive>
              ...
              <tfoot>
                <tr>
                  <th>All<input type="checkbox" /></th> {/* Checkbox in the footer */}
                  <th>#</th>
                  <th>Name</th>
                  <th>Entry Id</th>
                  <th>Line</th>
