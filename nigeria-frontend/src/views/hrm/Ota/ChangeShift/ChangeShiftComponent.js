...
tableRef.current = $('#changezone').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      navigate('/login');
        ...
        const fetchOperatorData = () => {
            fetch(`${config.apiUrl}/ota/getShiftdata`,{headers: customHeaders})
  ...
  const handleMove = (entryIds) => {
    ...
    navigate(`/admin/hrm/ota/multipleshiftchangeota?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `${config.apiUrl}/ota/get_shiftdata_search?where=${where}`,
      method: 'GET',
      headers: customHeaders,
  })
  .done((response) => {
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">Change Shift[Filter]</CardTitle>
