...
const [isActive, setActive] = useState(false);
  ...
  const handleEdit = (id) => {
    navigate(`/admin/master/editline/${id}`);
  };
  ...
  const handleDelete = (id) => {
    ...
    if (tableRef.current) {
      tableRef.current.destroy();
       ...
       fetch(`${config.apiUrl}/linedelete/${id}`, {
        method: 'DELETE',
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">Line Master <Link to="/admin/master/addnewline" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new
            </Link></CardTitle>
              </CardHeader>
              <CardBody>
