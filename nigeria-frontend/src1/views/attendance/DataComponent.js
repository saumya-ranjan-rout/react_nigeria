...
useEffect(() => {
    ...
    if (!isLoggedIn) {
      ...
      $.ajax({
        url: `${config.apiUrl}/process-tables`,
        method: 'GET',
  ...
  return (
    <>
      ...
      <div className="message-box">
        ...
        <h1 className="display-3 textblack" >Thank You!</h1>
        <p className="lead textblack"><strong>Data is updated successfully.</strong> for further instructions check worker details.</p>
        <hr />
        <div className="buttons-con">
          <div className="action-link-wrap">
            
            <a onClick={() => { navigate('/admin/dashboard'); }} className="link-button link-back-button">Go to Home Page</a>
          </div>
        </div>
      </div>
    </>
