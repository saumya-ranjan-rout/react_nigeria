...
function OperatorEfficiencyReport() {
 ...
 const token = localStorage.getItem('token');
  const customHeaders = {
    ...
    const handleSubmit = (event) => {
        ...
        //alert(jsonData);
        $.ajax({
           url: `${config.apiUrl}/get_supervisor_search`,
            method: "POST",
    ...
    const initializeTable = (timesheet, formattedDate) => {
      ...
      {
        extend: 'csv',
        filename: `NON-BRAID OPERATOR EFFICIENCY REPORT Date ${formattedDate}`, // Set your desired filename here
      },
    ],
    ...
    useEffect(() => {
 setLoading(true);
        document.title = 'Operator Efficiency Report';
        ...
        if (!isLoggedIn) {
            ...
            $.ajax({
                url: `${config.apiUrl}/get_supervisoreff_default`,
                method: "GET",
                headers: customHeaders,
                data: [],
                processData: false,
      ...
      <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
               <CardTitle tag="h5"> Operator Efficiency
                <hr></hr>
                Date Range
                 <hr></hr>
               </CardTitle>
...
export default OperatorEfficiencyReport;
