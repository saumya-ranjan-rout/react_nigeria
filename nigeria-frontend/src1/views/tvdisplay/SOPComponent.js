...
const [isActive, setActive] = useState(false);
  ...
  const handleView = (id) => {
   navigate(`/admin/tvdisplay/viewsop/${id}`, { state: { itemId: id } });
  };

 
  const handleDelete = (id) => {
  const confirmDelete = window.confirm('Do You Really Want To Delete SOP?');
  if (confirmDelete) {
    fetch(`${config.apiUrl}/sopdelete/${id}`, {
      method: 'DELETE',
      headers: customHeaders,
    })
      .then((response) => response.json())
      .then((data) => {
        console.log('SOP deleted:', data);
            ...
            },
            {
              ...
              render: function (data, type, row, meta) {
                ...
                return `
                  ...
                  <button class="btn btn-sm btn-danger" onclick="handleDelete(${id})" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button>
                `;
              },
            },
            {
              ...
              render: function (data, type, row, meta) {
                ...
                return `
                  ...
                  <button class="btn btn-sm btn-info" onclick="handleView(${id}, '${row.item_description}')" title="View"><i class="fa fa-eye" aria-hidden="true"></i> View </button>
        ...
        <Row>
          ...
          <Col md="12">
            <Card className="card-user">
              <CardHeader>
                <CardTitle tag="h5">SOP & Safety  <Link to="/admin/tvdisplay/addnewsop" className="btn btn-primary mr-2" style={{ margin: '0', padding: '5px', fontSize: '12px' }}>
              Add new
            </Link></CardTitle>
              </CardHeader>
              <CardBody>
