...
function AddEmployeeTimesheetNBraid() {
...
const handleSubmit = (event) => {
  ...
  if (roleId === '5') { // Compare roleId as string
    console.log('Inside roleId === "5" condition'); // Log inside the condition for debugging
    $.ajax({
    url: `${config.apiUrl}/getaddemployeetimesheetfilterdatanbraidota`,
    method: 'POST',
    ...
    $.ajax({
    url: `${config.apiUrl}/getaddemployeetimesheetfilterdatanbraidotaoplogin/${userid}`,
    method: 'POST',
    headers: customHeaders,
    data: jsonData,
    processData: false,
    ...
    console.warn('Unhandled roleId:', roleId);
  }
};
...
const handleSubmitnew = (event) => {
  ...
  tableRows.forEach((row) => {
    ...
    if (completes !== '') {
      const rowData = {
        ...
        joindate: $row.find('td:eq(15)').text(),
  ...
  if (roleId === '5') {
   ...
   $.ajax({
    url: `${config.apiUrl}/insertemployeetimesheetnbraidotadataoplogin/${userid}`,
    type: 'POST',
    headers: customHeaders,
...
// Destroy the existing DataTable instance (if it exists)
 ...
 // Fetch section options from API
    const fetchSectionOptions = () => {
      $.ajax({
        url: `${config.apiUrl}/getSectionOptionsOpLogin/${userid}`,
        method: 'GET',
        headers: customHeaders,
        success: function (response) {
          setSectionOptions(response);
                ...
                />
                    ...
                    <div>
                      <h6 className="header-title">
                        <span className="textblue">{lines}</span>-><span className="textred">{sectionnames}[{hours}] ->{shiftts}</span>
                      </h6>
                    </div>
                      ...
                      {wnames.length > 0 && (
                        <div>
                          {wnames.split(', ').map((worker, index) => (
                            <p key={index}>
                              {worker} worker is working for another product in <span className='textblue'>{hours}</span> hour.
                            </p>
                          ))}
                        </div>
                      )}
                    ...
                    <div className="row space">
                      <div className="col-sm-6">
                        <input className="form-control" id="myInput" type="text" placeholder="Search.." value={searchValue} onChange={handleSearch} />
                      </div>
                      <div className="col-sm-3">&nbsp;</div>
                      <div className="col-sm-2">
                        <input className="btn btn-primary btn-sm" value="Replicate Qty" id="btn" readonly="" style={{ width: '150px' }}  onClick={handleReplicateClick} />
                      </div>
                    </div>
                    ...
                    <div className="form-group row">
                      ...
                      <div className="col-sm-1">
                        <input type="submit" id="submit-data" className="btn btn-success margin-top" value="Save" data-loading-text="Adding..." />
                      </div>
                    </div>
                  </form>
