...
export function Employees() {
  ...
  const handleView = (id) => {
    navigate(`/admin/hrm/ota/viewworkerota/${id}`);
  };
  const ConvertToOp = (id) => {
    navigate(`/admin/hrm/ota/changetooperatorota/${id}`);
  };
  ...
  setModal(true);
  try {
    const response = await axios.get(`${config.apiUrl}/ota/operator_data_single/${id}`, { headers: customHeaders });
...
const [formData, setFormData] = useState({
  ...
  const handleSubmit = (event) => {
    ...
    $.ajax({
      url: `${config.apiUrl}/ota/getSectionName/${formData.sectionId}`,
      method: 'GET',
    ...
    $.ajax({
        url: `${config.apiUrl}/ota/get_employees_nbraid?where=${where}`,
        method: 'GET',
        headers: customHeaders,
    })
    .done((response) => {
...
if ($.fn.DataTable.isDataTable('#employee')) {
  ...
  tableRef.current = $('#employee').DataTable({
      ...
      },
      {
          ...
          <button class="btn btn-sm btn-danger" style="color:#fff;font-size:15px;padding:2px 3px;margin-bottom:2px;width :30px;" onclick="handleDelete('${id}')"  title="Delete"><i class="fa fa-trash"></i></button>
           <button class="btn btn-sm btn-primary" style="color:#fff;font-size:15px;padding:2px 3px;margin-bottom:2px;width :30px;" onclick="ConvertToOp(${id})" title="Convert"><i class="fa fa-user"></i></button>
  ...
  useEffect(() => {
    ...
    fetchLines();
       //Default employees data fetch
        const fetchData = () => {
        $.ajax({
            url: `${config.apiUrl}/ota/get_employeess_nbraid`,
            method: 'GET',
            headers: customHeaders,
            success: function (response) {
            // Set the fetched data in the state
        ...
        fetchData();
         ...
         const fetchDate = () => {
          $.ajax({
              url: `${config.apiUrl}/ota/get_employeess_nbraid_date`,
              method: 'GET',
              headers: customHeaders,
              success: function (response) {
              // Set the fetched data in the state
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDelete) {
      $.ajax({
        url: `${config.apiUrl}/ota/delete_user/${id}`,
        method: 'DELETE',
        headers: customHeaders,
        success: function (response) {
          alert(response);
...
const handleTransferConfirmation = async () => {
    try {
      ...
      const response = await axios.post(`${config.apiUrl}/otaemployeetransfer`, transferData, {
        headers: customHeaders
      });
    ...
    <div className="content">
        <Row>
         ...
         <Modal isOpen={modal} toggle={toggleModal}>
          ...
          <ModalBody>
            <p>Are you sure you want to transfer this employee to <span className="badge-success">IKEJA</span> Site ?</p>
